...
def create_upload_images_dir():
    if not os.path.exists("upload_images"):
        os.mkdir("upload_images")


def run():
    st.title("Fridge Items🍍of Natural Elements🍅 Classification")
   # Create "upload_images" directory if it doesn't exist
    create_upload_images_dir()
    ...
    if img_file is not None:
        ...
        try:
            ...
            st.error(traceback.format_exc())  # Display detailed traceback for debugging
            return

        # Only proceed if image is successfully uploaded
        if img_file is not None:
